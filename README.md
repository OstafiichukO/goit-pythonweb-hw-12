...
- Щоб працювати з Redis із Python, потрібно встановити один із пакетів для
  роботи, наприклад redis:
  ```shell
  poetry add redis
  ```
...
- Для контролю терміну дії кешу використаємо модуль
  ...
  ```
- Для кешування функції `get_current_user` з модуля
  ...
  - перед запитом в БД перевіряємо чи користувач в кеші і повертаємось з функції
    без запиту
...
2. Сервер генерує унікальний токен скидання пароля та надсилає лист із
   посиланням на скидання пароля на адресу електронної пошти користувача.
   Посилання вмикає маркер скидання пароля як параметр.

3. Коли користувач натискає на посилання скидання пароля, він потрапляє на API
   зміни пароля.
...
5. Якщо токен вірний, сервер оновлює пароль користувача та повідомляє про це
   користувача.
...
- змінимо маршрут `/avatar` в [src/api/users.py](src/api/users.py), щоб він
  працював тільки для адміністраторів
...
Конфіденційні дані та налаштування зберігаємо у файлі `.env`, який не включаємо
